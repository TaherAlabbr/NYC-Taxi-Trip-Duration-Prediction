...
def remove_outliers_iqr(df: pd.DataFrame, multiplier, columns: list, iqr_boundries=None)  -> pd.DataFrame:
    """
    ...
    iqr_boundries : list of tuple, optional
    ...
    if not iqr_boundries:
        ...
        for col in columns:
            ...
            iqr_boundries.append((lower_bound, upper_bound))
            df = df[(df[col] >= lower_bound) & (df[col] <= upper_bound)]
    else:
        for iqr, col in zip(iqr_boundries, columns):
            df = df[(df[col] >= iqr[0]) & (df[col] <= iqr[1])]
    return df, iqr_boundries
